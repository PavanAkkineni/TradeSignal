...
def get_ibm_weekly_adjusted():
    """
    Fetch weekly adjusted data for IBM stock from Alpha Vantage API.
    Gets full historical data (20+ years available, we need 3 years minimum).
    ...
    print(f"Fetching IBM Weekly Adjusted data...")
    print(f"Symbol: IBM")
    print(f"Data: Full historical (20+ years available)")
    print(f"Required: 3 years minimum (156 weeks)")
    print("-" * 50)
    ...
    try:
        ...
        if "Weekly Adjusted Time Series" in data:
            ...
            print(f"✅ Successfully retrieved {len(time_series)} weekly data points")
            ...
            if dates:
                ...
                print(f"Years of data: {years:.1f} years ({weeks} weeks)")
                
                # Display sample data (most recent week)
                print("\nSample data (most recent week):")
                print(f"Week ending: {latest}")
...
def save_data_to_file(data, filename=None):
    ...
    if filename is None:
        ...
        filename = f"ibm_weekly_adjusted_{timestamp}.json"
    
    # Save in the same directory as this script (Weekly folder)
...
def main():
    """Main function to fetch and save IBM weekly adjusted data."""
    try:
        # Fetch full historical data
        data = get_ibm_weekly_adjusted()
        
        # Save to file
        save_data_to_file(data)
        
        print("\n✅ Done! Weekly adjusted data fetched and saved successfully.")
        print("📊 Use cases:")
        print("   - Medium-term trend identification")
        print("   - Noise reduction vs daily data")
        print("   - Position trading (weeks to months)")
        print("   - Weekly technical indicators (EMA 13/26/52)")
